...
export default mutation(async ({ db, auth }): Promise<any> => {
    ...
    if (user !== null) {
        ...
        if (user.name != identity.name) {
            db.patch(user._id, { name: identity.name!, email: identity.email!, phone: identity.phoneNumber });
        }
        return user._id;
    }
